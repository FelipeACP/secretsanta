...
import { Code, Gift, Heart, Rows, Star } from "@phosphor-icons/react";
...
export function Home() {
  ...
  >("secretSantaParticipants", {}, migrateParticipants);
  ...
  const menuItems = [
    <div className="flex flex-col space-y-2 lg:flex-row lg:space-y-0 lg:space-x-2">
      <MenuItem
        key={`vanity`}
        to="https://bsky.app/profile/mael.dev"
      ...
      <MenuItem
        key={`sponsor`}
        to="https://github.com/sponsors/arcanis?frequency=one-time&sponsor=arcanis"
        icon={<Heart className={`text-red-700`} weight={`fill`} />}
      >
        {t(`home.sponsor`)}
      </MenuItem>
    </div>,
    <MenuItem
      key={`giftSuggestions`}
      to="#/gift-suggestions"
      icon={<Gift className={`text-green-600`} weight={`bold`} />}
    >
      {t(`giftSuggestions.menuTitle`)}
    </MenuItem>,
  ];
  ...
  const toggleViewButton = (
    <button
      ...
      title={t(
        isTextView
          ? "participants.switchToFormView"
          : "participants.switchToTextView"
      )}
    >
      {isTextView ? (
                    ...
                    components={{
                      ...
                      githubLink: (
                        <a
                          ...
                          href="https://github.com/arcanis/secretsanta/"
                          target="_blank"
                        />
                      ),
                      exampleLink: (
                        <Link
                          className="text-blue-500 underline"
                          to="/pairing?from=Simba&to=c1w%2FUV9lXC12U578BHPYZhXxhsK0fPTqoQDU9CA7W581P%2BM%3D"
                        />
                      ),
                    }}
                  />
